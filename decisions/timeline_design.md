...
## The history view's mandate got clearer, not narrower (2026-08-25)

Moved here from `decisions/parkinglot.md` 2026-08-25: it is a conclusion
ABOUT the ribbon, prompted by §8 but not part of it (Track B, not Track A).

A consequence worth recording, because it points the opposite way from where
§7e was heading. Removing open loops from the ribbon's concern does **not**
license dropping LOW blocks from history. The case Scott wants protected —
"something you did not spend a lot of time on, but is actually important and
you want to find it" — is not a parked tab. It is the Google Keep specimen
from `eviction-fallback-tedium`: genuinely visited, briefly, and now wanted
back.

What the lot changes is the *composition* of the ribbon's LOW band, not its
size: **every LOW block in the ribbon is now a real visit** rather than a
mixture of real visits and never-read deferrals. The population gets cleaner,
not smaller — which is a stronger argument for keeping it browsable than
generosity toward noise.

This puts §7e's band-drop ladder in direct conflict with Scott's stated rule:
"I want them to be narrow and not in your face, but they should be
browsable." The ladder currently drops LOW *entirely* at zoom-out, and
`band-drop-absence` already flags the failure mode. Those two cannot both
hold. Read this as resolving that watch item toward **a floor of presence,
however thin, over a clean disappearance** — deliberately not built here, but
no longer an open question of principle.
...
about the RIBBON, not the strip — zoom anchoring, cross-day loading, band
ladders, the coordinate system, panning. They moved here when
`decisions/tabmanager.md` closed, since this is the ribbon's decision log and
that work is Track B (ongoing history-surface evolution), not Track A.
Corresponding rules: `spec/ribbon.md` (§7c-ribbon–§7h), which keeps the §7x
numbering because ~85 code comments cite it.
