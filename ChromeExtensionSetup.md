...
* **Gap hours (agreed 2026-07-15):** empty wall-clock time draws as its hour ticks, compressed and uniform — every whole hour falling in a gap between blocks gets a fixed `GAP_HOUR_PX` (44px) slot with its normal tick + label, so a 4-hour absence reads as four countable, evenly spaced ticks (12pm 1pm 2pm 3pm): self-decoding, no start/stop math, ~1/12 the width of an attended hour. A gap crossing no hour boundary still draws nothing. This replaced both the original clamp-to-edge behavior (real data: a 4-hour absence clamped 12–3pm onto one x, shingling the labels — the parked "gap marker" watch-list item) and a proposed fixed-width break glyph with a 30-min threshold (rejected: a glyph must be decoded — "is that 2h or 3h away?" — and needs an arbitrary threshold; countable ticks need neither). Consequences: label stacking is structurally impossible (hours inside blocks interpolate, hours inside gaps own real space, the clamp case no longer exists); the axis never omits an hour; and since fence runs split at `VISIT_GAP_MS` (see fence bullet), gap slots inside a collapsed fence occur only in the freak sub-5-min hour-straddle case. Each gap region carries an invisible hover plate with the exact span ("away 11:23 AM – 3:12 PM · 3h 49m").
