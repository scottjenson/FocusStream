...
* **Container qualification:** same-`tabId` fragments chain when the return gap is < `VISIT_GAP_MS`, OR < `AUDIO_BOOKEND_GAP_MS` (30 min, provisional) when both bookend fragments are audible-dominated (`audibleMs` ≥ 50% of duration); tabIds don't survive browser restarts (rare, accepted). The chain containerizes when: **summed score ≥ MEDIUM** (2026-07-18; tier = band of the sum); **≥1 interruption** exists — a foreign child in the span OR a departure boundary, i.e. a non-final fragment ending `tab_hidden` (2026-07-16; machinery boundaries never count, so continuous same-tab reading can't self-containerize); and, below HIGH, **anchor dominance** holds — anchor sum > children sum (a weak anchor framing stronger children is a launcher; the children are the story; dominance deliberately not applied ≥ HIGH). Overlapping qualifying chains: higher sum wins.
* **Container display (2026-07-17):** a container paints exactly like a solid block of its tier — same fill, rim, hover. Children are **cut out** of the interior: drawn on top at time-proportional positions in their own tier paint (capped at MEDIUM — containment frames, never confers, never destroys), separated by a 2px page-background seam; no fences or labels inside (hover only); the container stretches if min-width children would overflow. Zero-children containers need no special case (the tooltip's "(interruptions outside the browser)" line marks them). Click opens the top-scoring fragment; the tooltip lists the anchor's pages, with span and counts in its debug section. Pipeline: after merging, before fencing.
* **Atomicity — what a thread may never swallow (unified 2026-07-18; guards agreed 2026-07-15):** (1) an **individually-HIGH session owns its story** — it never joins a merge, never chains, and rejects any chain whose span would cover it; (2) a **side-quest is foreign by definition** (§5) — it never merges into its neighbors, never fences (MEDIUM+ structurally cannot hide), and inside a container stays a visible cut-out child; (3) **foreign hosts never merge** — same-host is a necessary condition for every join, and host = hostname, not registrable domain (`mail.google.com` ≠ `docs.google.com`).
...
* **Containers:** two long videos in one tab <30 min apart chain into a "YouTube container" (audio-bookend false positive). ~~Meet titles misfire the site-name heuristic ("Scott")~~ resolved 2026-07-18 by invariance naming ("Meet" is the invariant segment).
