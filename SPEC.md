...
* **The thread is the display atom (2026-07-18).** A thread is a maximal chain of same-host fragments in one tab, joined across machinery and departure boundaries (§5 boundary taxonomy), broken by absences, foreign hosts, and atomicity (below). Score/tier = band of the **summed** fragments — add up, then judge. A single unmerged session is a thread of length 1; no special rules. Display-time only; capture stays granular.
* **Thread assembly — merging:** two joins build visit blocks before fencing, scored and banded on the merged totals. (1) *Machinery join* (2026-07-18): same-tab same-host neighbors whose boundary is `spa_navigation`/`navigated` (< 30s gap sanity bound) merge **regardless of band** — three back-to-back YouTube videos are one watch. (2) *Absence-bridge join* (2026-07-15): consecutive same-host **LOW** blocks with gaps < `VISIT_GAP_MS` (5 min) merge — noise-level grazing may bridge small absences; anything with signal may not. A still-LOW merged visit stays fence-eligible; merged visits carry their last member's `endReason` (so merged fragments can testify as departures). Click opens the top-scoring member; tooltip reports the page count.
...
* **Week strip vs. thread bands:** strip tiers are raw per-session bands while the ribbon shows thread-level bands — every grouping improvement widens the gap (meeting-heavy days skyline lower than their ribbon). If it grates, extract thread banding (not layout) into a shared step.
